            ...
            if self.calendar_rooms.get(room.room_id):
                for calendarid in self.calendar_rooms.get(room.room_id):
                    ...
                    for event in events:
                        ...
                        s = s + '</b> ' + event.title + \
                            " " + (event.notes or '')
                        await bot.send_html(room, event, s, s)
        elif len(args) == 2:
            if args[1] == 'list':
                await bot.send_text(room, event, f'Calendars in this room: {self.calendar_rooms.get(room.room_id) or []}')
            elif args[1] == 'poll':
            ...
            if args[1] == 'add':
                ...
                if self.calendar_rooms.get(room.room_id):
                    if calid not in self.calendar_rooms[room.room_id]:
                        ...
                        await bot.send_text(room, event, 'This teamup calendar already added in this room!')
                        return
                else:
                ...
                await bot.send_text(room, event, 'Added new teamup calendar to this room')
            if args[1] == 'del':
                bot.must_be_admin(room, event)
                ...
                bot.save_settings()
                self.setup_calendars()
                await bot.send_text(room, event, 'Removed teamup calendar from this room')
            if args[1] == 'apikey':
                bot.must_be_owner(event)
