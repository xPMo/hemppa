...
class MatrixModule(BotModule):
    ...
    async def message_cb(self, room, event):
        ...
        if(target_room):
            ...
            await self.bot.send_text(target_room, event, f'<{sendernick}> {event.body}', msgtype="m.text", bot_ignore=True)
        else:
    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) == 2:
            if args[0] == 'bridge':
                ...
                if room_to_bridge:
                    await bot.send_text(room, event, f'Bridging {room_to_bridge.display_name} here.')
                    self.bridges[room.room_id] = roomid
                    bot.save_settings()
                else:
                    await bot.send_text(room, event, f'I am not on room with id {roomid} (note: use id, not alias)!')
                ...
                for src_id, tgt_id in self.bridges.items():
                    if i == idx:
                        ...
                        await bot.send_text(room, event, f'Unbridged {src_id} and {tgt_id}.')
                        bot.save_settings()
                        return
                    i = i + 1
