...
class MatrixModule(BotModule):
    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) > 1 and args[1] in ['set', 'setserver']:
            ...
            if len(args) < 3:
                self.host = None
                return await bot.send_text(room, event, f'Usage: !{args[0]} {args[1]} [host] ([port])')
            self.host = args[2]
            if len(args) > 3:
            ...
            if not self.port:
                self.port = 64738
            bot.save_settings()
            return await bot.send_text(room, event, f'Set server settings: host: {self.host} port: {self.port}')

        self.logger.info(f"room: {room.name} sender: {event.sender} wants mumble info")
        if not self.host:
            return await bot.send_text(room, event, f'No mumble host info set!')
        ...
        try:
            ...
            # [5] = users
            # [6] = max users
            await bot.send_text(room, event, f'{self.host}:{self.port} (v{version}): {ret[5]} / {ret[6]} (ping: {ping}ms)')
        except socket.gaierror as e:
            self.logger.error(f"room: {room.name}: mumble_ping failed: {e}")
            await bot.send_text(room, event, f'Could not get get mumble server info: {e}')
