    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) == 3:
            if args[1] == "appid":
                ...
                self.app_id = args[2]
                bot.save_settings()
                await bot.send_text(room, event, 'App id set')
                return

        if len(args) > 1:
            if self.app_id == '':
                await bot.send_text(room, event, 'Please get and set a appid: https://products.wolframalpha.com/simple-api/documentation/')
                return
            ...
            if full[0] and 'full' in args[0]:
                ...
                html = plain
            await bot.send_html(room, event, html, plain)
        else:
            await bot.send_text(room, event, 'Usage: !wa <query>')
