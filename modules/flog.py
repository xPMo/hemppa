...
#                print(f'Got {len(flights)} flights at {station}')
    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) == 1 and args[0] == "!flog":
            if room.room_id in self.station_rooms:
                ...
                await bot.send_text(room, event, 'No OGN station set for this room - set it first.')
            ...
            if args[1] == 'rmstation':
                ...
                await bot.send_text(room, event, f'Cleared OGN station for this room')

            elif args[1] == 'status':
                print(self.logged_flights)
                print(self.fb.device_cache)
                bot.must_be_admin(room, event)
                await bot.send_text(room, event, f'OGN station for this room: {self.station_rooms.get(room.room_id)}, live updates enabled: {room.room_id in self.live_rooms}')
                ...
                bot.save_settings()
                await bot.send_text(room, event, f'Sending live updates for station {self.station_rooms.get(room.room_id)} to this room')

            elif args[1] == 'rmlive':
                bot.must_be_admin(room, event)
                self.live_rooms.remove(room.room_id)
                bot.save_settings()
                await bot.send_text(room, event, f'Not sending live updates for station {self.station_rooms.get(room.room_id)} to this room anymore')
            ...
            if coords:
                await bot.send_location(room, event, f'{registration} ({coords["utc"]})', coords["lat"], coords["lng"])
            else:
                await bot.send_text(room, event, f'No Flarm ID found for {registration}!')
            ...
            if args[1] == 'station':
                ...
                bot.save_settings()
                await bot.send_text(room, event, f'Set OGN station {station} to this room')
    ...
    async def show_flog(self, bot, room, station):
        data = self.fb.get_flights(station)
        if data:
            await bot.send_html(room, event, self.html_flog(data, False), self.text_flog(data, False))
        else:
            await bot.send_text(room, event, f"Failed to get flight log for {station}")
