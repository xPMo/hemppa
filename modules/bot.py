...
class MatrixModule(BotModule):
    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) == 2:
            if args[1] == 'quit':
                ...
                await self.version(bot, room, event)
            elif args[1] == 'reload':
                await self.reload(bot, room, event)
            elif args[1] == 'status':
                await self.status(bot, room, event)
            elif args[1] == 'stats':
                await self.stats(bot, room, event)
            elif args[1] == 'leave':
                await self.leave(bot, room, event)
            elif args[1] == 'modules':
                await self.show_modules(bot, room, event)
            elif args[1] == 'export':
        ...
        #    await bot.send_text(room, event, 'Unknown command, sorry.')
    ...
    async def get_ping(self, bot, room, event):
        ...
        pong = await bot.send_text(room, event, 'Pong!')
    ...
    async def leave(self, bot, room, event):
        ...
        await bot.send_text(room, event, f'By your command.')
        await bot.client.room_leave(room.room_id)

    async def stats(self, bot, room, event):
        ...
        await bot.send_text(room, event, f'I\'m seeing {usercount} users in {roomcount} rooms.'
                f' Top ten homeservers (out of {hscount}): {homeservers}')

    async def status(self, bot, room, event):
        systime = time.time()
        ...
        return await bot.send_text(room, event, f'Uptime: {uptime} - System time: {systime} '
                f'- {enabled} modules enabled out of {len(bot.modules)} loaded.')

    async def reload(self, bot, room, event):
        bot.must_be_owner(event)
        msg = await bot.send_text(room, event, f'Reloading modules...')
        bot.stop()
        bot.reload_modules()
        bot.start()
        # update event
    ...
    async def version(self, bot, room, event):
        await bot.send_text(room, event, f'Hemppa version {bot.version} - https://github.com/vranki/hemppa')

    async def quit(self, bot, room, event):
        bot.must_be_owner(event)
        await bot.send_text(room, event, f'Quitting, as requested')
    ...
    async def enable_module(self, bot, room, event, module_name):
        ...
        if bot.modules.get(module_name):
            ...
            bot.save_settings()
            return await bot.send_text(room, event, f"Module {module_name} enabled")
        return await bot.send_text(room, event, f"Module with name {module_name} not found. Execute !bot modules for a list of available modules")
    ...
    async def disable_module(self, bot, room, event, module_name):
        ...
        if bot.modules.get(module_name):
            ...
            try:
                ...
                return await bot.send_text(room, event, f"Module {module_name} cannot be disabled.")
            except Exception as e:
                return await bot.send_text(room, event, f"Module {module_name} was not disabled: {repr(e)}")
            module.matrix_stop(bot)
            bot.save_settings()
            return await bot.send_text(room, event, f"Module {module_name} disabled")
        return await bot.send_text(room, event, f"Module with name {module_name} not found. Execute !bot modules for a list of available modules")

    async def show_modules(self, bot, room, event):
        ...
        await bot.send_text(room, event, modules_message)
    ...
    async def last_logs(self, bot, room, event, target):
        ...
        for key in [target, f'module {target}']:
            ...
            return await bot.send_text(msg_room, event, f'Unknown module {target}, or no logs yet')

        if count:
            logs = logs[count:]
        logs = '\n'.join([self.loghandler.format(record) for record in logs])

        return await bot.send_html(msg_room, event, f'<strong>Logs for {key}:</strong>\n<pre><code class="language-txt">{escape(logs)}</code></pre>', f'Logs for {key}:\n' + logs)
    ...
    async def rooms(self, bot, room, event):
        ...
        await bot.send_text(room, event, output)
