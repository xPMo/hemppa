...
class MatrixModule(BotModule):
        ...
        if len(args) >= 1:
            if args[0] == "toot":
                ...
                if accesstoken:
                    ...
                    await bot.send_text(room, event, tootdict['url'])
                else:
                    await bot.send_text(room, event, f'{event.sender} has not logged in yet with the bot. Please do so.')
                return
        ...
        if len(args) == 5:
            if args[0] == "roomlogin":
                ...
                if roomid:
                    ...
                    await bot.send_text(room, event, f'Unknown room alias {roomalias} - invite bot to the room first.')
                return
        if len(args) == 1:
            if args[0] == "status":
                ...
                await bot.send_text(room, event, out)
            if args[0] == "logout":
                if event.sender in self.logins.keys():
                    ...
                    bot.save_settings()
                    await bot.send_text(room, event, f'{event.sender} login data removed from the bot.')
            if args[0] == "roomlogout":
                bot.must_be_admin(room, event)
                if room.room_id in self.roomlogins.keys():
                    del self.roomlogins[room.room_id]
                    bot.save_settings()
                    await bot.send_text(room, event, f'Login data for this room removed from the bot.')
                else:
                    await bot.send_text(room, event, f'No login found for room id {room.room_id}.')
            if args[0] == "clear":
                bot.must_be_owner(event)
                self.logins = dict()
                self.roomlogins = dict()
                bot.save_settings()
                await bot.send_text(room, event, f'All Mastodon logins cleared')
            if args[0] == "setpublic":
                bot.must_be_owner(event)
                self.public = True
                bot.save_settings()
                await bot.send_text(room, event, f'Mastodon usage is now public use')
            if args[0] == "setprivate":
                bot.must_be_owner(event)
                self.public = False
                bot.save_settings()
                await bot.send_text(room, event, f'Mastodon usage is now restricted to bot owners')
    ...
    async def register_app_if_necessary(self, bot, room, instanceurl):
        if not instanceurl in self.apps.keys():
            ...
            bot.save_settings()
            await bot.send_text(room, event, f'Registered Mastodon app on {instanceurl}')
    ...
    async def login_to_account(self, bot, room, mxid, roomid, instanceurl, username, password):
        ...
        if mxid:
            ...
            await bot.send_text(room, event, f'Logged Matrix user {mxid} into {instanceurl} as {username}')
        elif roomid:
            self.roomlogins[roomid] = [username, access_token, instanceurl]
            await bot.send_text(room, event, f'Set room {roomid} Mastodon user to {username} on {instanceurl}')
