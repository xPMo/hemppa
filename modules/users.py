    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) == 1:
            if args[0] == 'stats' or args[0] == 'roomstats':
                ...
                total = len(allusers)
                if total == 0:
                    await bot.send_text(room, event, "I don't see any users. How did this happen?")
                    return
                ...
                await bot.send_text(room, event, reply)
                return
        if len(args) == 2:
            if args[0] == 'list' or args[0] == 'listall':
                ...
                if len(users):
                    await bot.send_text(room, event, ' '.join(users))
                else:
                    await bot.send_text(room, event, 'No matching users found!')
                return
            if args[0] == 'kick':
            ...
            if args[0] == 'classify':
                if args[1] == 'list':
                    await bot.send_text(room, event, f'Classes in use: {self.classes}.')
                    return
        elif len(args) == 4:
            if args[0] == 'classify':
                if args[1] == 'add':
                    ...
                    await bot.send_text(room, event, f'Added class {name} pattern {pattern}.')
                    bot.save_settings()
                    return
        elif len(args) == 3:
            if args[0] == 'classify':
                if args[1] == 'del':
                    ...
                    await bot.send_text(room, event, f'Deleted class {name}.')
                    bot.save_settings()
                    return

        await bot.send_text(room, event, 'Unknown command - please see readme')
