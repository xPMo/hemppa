    ...
    async def file_cb(self, room, event):
        try:
            ...
            if room.room_id in self.printers:
                ...
                await self.bot.send_text(room, event, f'Printing file on {printer}..')
                ...
                await self.bot.send_text(room, event, f'Printing failed, sorry. See log for details.')
    ...
    async def matrix_message(self, bot, room, event):
        ...
        if len(args) == 1:
            if args[0] == 'list':
                ...
                await bot.send_text(room, event, f'Deleted printer from this room.')
                bot.save_settings()
        ...
        if len(args) == 2:
            if args[0] == 'setroomprinter':
                ...
                if printer in printers:
                    await bot.send_text(room, event, f'Printing with {printer} here.')
                    self.printers[room.room_id] = printer
                    bot.save_settings()
                else:
                    await bot.send_text(room, event, f'No printer called {printer} in your CUPS.')
            if args[0] == 'setpapersize':
                self.paper_size = args[1]
                bot.save_settings()
                await bot.send_text(room, event, f'Paper size set to {self.paper_size}.')
